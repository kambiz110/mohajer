...
{
    public interface IGetProductWithMediaType
    {
        ResultDto<ProductForUserDto> Exequte(long type, int PageSize, int PageNo);
    }
    public class GetProductWithMediaType : IGetProductWithMediaType
    {
        private readonly MohajerContext _mohajerContext;
        private readonly IMapper _mapper;

        public GetProductWithMediaType(MohajerContext mohajerContext, IMapper mapper)
        {
            _mohajerContext = mohajerContext;
            _mapper = mapper;
        }

        public ResultDto<ProductForUserDto> Exequte(long type, int PageSize, int PageNo)
        {
            int rowCount = 0;

            List<Prodoct> result = new List<Prodoct>();

            result = _mohajerContext.Prodoctes.Where(p => p.IsRemove == false && p.IsActive == true && p.CountentType == type).ToPaged(PageNo, PageSize, out rowCount).ToList();
