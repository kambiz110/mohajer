...
{
    public interface IGetProductesWithCategoryId
    {
        ResultDto<ProductForUserDto> Exequte(long categoryId, int PageSize, int PageNo);
    }
    public class GetProductesWithCategoryId : IGetProductesWithCategoryId
    {
        private readonly MohajerContext _mohajerContext;
        private readonly IMapper _mapper;

        public GetProductesWithCategoryId(MohajerContext mohajerContext, IMapper mapper)
        {
            _mohajerContext = mohajerContext;
            _mapper = mapper;
        }

        public ResultDto<ProductForUserDto> Exequte(long categoryId, int PageSize, int PageNo)
        {
            int rowCount = 0;

            List<Prodoct> result = new List<Prodoct>();

            result = _mohajerContext.Prodoctes.Where(p => p.IsRemove == false &&p.IsActive==true && p.CategoryId== categoryId).ToPaged(PageNo, PageSize, out rowCount).ToList();
