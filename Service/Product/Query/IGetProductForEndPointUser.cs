...
{
   public interface IGetProductForEndPointUser
    {
        ResultDto<ProductForUserDto> Exequte(int PageSize, int PageNo);
    }


    public class GetProductForEndPointUser : IGetProductForEndPointUser
    {
        private readonly MohajerContext _mohajerContext;
        private readonly IMapper _mapper;

        public GetProductForEndPointUser(MohajerContext mohajerContext, IMapper mapper)
        {
            _mohajerContext = mohajerContext;
            _mapper = mapper;
        }

        public ResultDto<ProductForUserDto> Exequte(int PageSize, int PageNo)
        {
            int rowCount = 0;

            List<Prodoct> result = new List<Prodoct>();

            result = _mohajerContext.Prodoctes.Where(p => p.IsRemove == false).ToPaged(PageNo, PageSize, out rowCount).ToList();
